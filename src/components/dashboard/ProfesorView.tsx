...
export default function ProfesorView() {
  ...
  // 2. Escuchar estado en tiempo real desde RTDB (status_dispositivos)
  useEffect(() => {
  ...
  // 3. Carga de Perfil del Profesor y Escucha de Alumnos del Aula
  useEffect(() => {
    if (!workingInstitutoId) return;
      ...
      if (user?.email) {
        ...
        const unsubProf = onSnapshot(qProf, (snap) => {
          if (!snap.empty) {
            ...
            return () => {
              off(dispositivosRef);
              console.log("🧹 Listener de dispositivos limpiado");
            };
          }
        });
        ...
        return () => {
          unsubProf();
          console.log("🧹 Listener de profesores limpiado");
        };
      }
    });
  ...
  // 4. Escuchar alertas para el contador
  useEffect(() => {
    if (!workingInstitutoId) return;
    ...
    const unsubscribe = onValue(alertasRef, (snapshot) => {
      ...
      if (data) {
        const lista = Object.entries(data)
          ...
          .filter(alert => {
            const device = alumnos.find(a => a.id === alert.deviceId);
            return device && device.InstitutoId === workingInstitutoId;
          });
        setAlertas(lista);
      } else {
        setAlertas([]);
    ...
    return () => {
      off(alertasRef);
      console.log("🧹 Listener de alertas_seguridad limpiado");
    };
  }, [workingInstitutoId, alumnos]);

  // 5. Acción de Blindaje Masivo (shield_mode_enable para todos los alumnos del aula)
  ...
  // 6. Acción Individual - Bloquear/Desbloquear dispositivo (shield_mode_enable)
  ...
  // 7. Verificar si dispositivo está online
  ...
  // 8. Obtener URL actual desde RTDB
  const getCurrentUrl = (deviceId: string) => {
    const rtInfo = realtimeStatusMap[deviceId];
    return rtInfo?.url_actual || 'Navegador inactivo';
  };

  // 9. Verificar si dispositivo está bloqueado por shield_mode
  ...
  // Mensaje si el profesor no tiene aula asignada
            ...
            return (
              <div key={al.id} className={`bg-[#0f1117] border rounded-[2.5rem] p-6 shadow-2xl hover:border-blue-500/50 transition-all group ${
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  {/* Botón BLOQUEAR/DESBLOQUEAR INDIVIDUAL */}
                  <button 
