...
import { ref, onValue, set, update, remove, query as queryRTDB, orderByKey, limitToLast, get, off } from 'firebase/database';
...
export default function SuperAdminView() {
  ...
  const handleDeleteDevice = async (deviceId: string) => {
    ...
    try {
      ...
      await remove(ref(rtdb, `status_dispositivos/${deviceId}`));
  ...
  // Escucha de Dispositivos combinada con status_dispositivos
  useEffect(() => {
    const devicesRef = ref(rtdb, 'dispositivos');
    const statusRef = ref(rtdb, 'status_dispositivos');
    
    const unsubDevices = onValue(devicesRef, (devicesSnapshot) => {
      const devicesData = devicesSnapshot.val() || {};
      
      onValue(statusRef, (statusSnapshot) => {
        const statusData = statusSnapshot.val() || {};
        const combined = Object.keys(devicesData).map(key => ({
          id: key,
          ...devicesData[key],
          ...statusData[key]
        }));
        ...
        const filtered = targetAulaId 
          ? combined.filter(d => d.aulaId === targetAulaId)
          : combined;
        setDevices(filtered);
      }, { onlyOnce: false });
    });
  ...
  const getOrCreateDevice = async () => {
    ...
    if (existingId) {
      ...
      await update(ref(rtdb, `status_dispositivos/${existingId}`), {
        InstitutoId: selectedConfig.instId,
        aulaId: selectedConfig.aulaId,
        seccion: selectedConfig.seccion.toUpperCase().trim(),
        estado: 'pending_hardware',
        lastSeen: Date.now()
      });
    ...
    await set(ref(rtdb, `status_dispositivos/${nextId}`), {
      lastSeen: Date.now(),
      estado: 'pending_hardware',
  ...
  const handleSaveStudent = async () => {
    ...
    try {
      ...
      await update(ref(rtdb, `status_dispositivos/${lastLinkedDevice.id}`), {
        alumno_asignado: studentName.trim(),
        estado: 'active',
        lastUpdated: Date.now()
      });
          ...
          {activeTab === 'dispositivos' && (
            <div className="space-y-10 animate-in fade-in">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {devices.map(device => (
                  <div key={device.id} className={`bg-[#0f1117] rounded-[2.5rem] border ${device.restrictions?.extremeSecurity || device.shield_mode_enable ? 'border-red-600' : 'border-slate-800'} p-8 shadow-xl transition-all hover:border-orange-500/50`}>
                    <div className="flex justify-between items-start mb-4">
                      <Smartphone size={20} className={device.restrictions?.extremeSecurity || device.shield_mode_enable ? 'text-red-500' : 'text-orange-500'} />
                    ...
                    {device.url_actual && (
                      <p className="text-blue-400 text-[8px] mt-2 truncate">🌐 {device.url_actual}</p>
                    )}
                    
                    {/* Botón "Activar Blindaje" ELIMINADO */}
