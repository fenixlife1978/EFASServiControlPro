...
import { 
  ...
  List, AlertTriangle, Calendar, Wifi, WifiOff, Users
} from 'lucide-react';
...
export default function InstitutionView() {
  ...
  const [activeSection, setActiveSection] = useState<'dashboard' | 'teachers' | 'settings' | 'security' | 'lists' | 'blocked'>('dashboard');
  ...
  // 2. Cargar dispositivos desde RTDB (dispositivos) y filtrar por institución
  useEffect(() => {
    if (!institutionId) return;
      ...
      if (snapshot.exists()) {
        ...
        const filtrados = Object.entries(data)
          .map(([key, value]: [string, any]) => ({ 
            ...
            lastSeen: value.lastSeen || value.ultimoAcceso
          }))
  ...
  // 3. Cargar aulas desde Firestore
  useEffect(() => {
    if (!institutionId) return;
  ...
  // 4. Filtrar dispositivos por aula seleccionada
  useEffect(() => {
    if (selectedAula && tablets.length > 0) {
      const enAula = tablets.filter(dev => {
        const devAula = String(dev.aulaId || "").trim().toUpperCase();
        const devSecc = String(dev.seccion || "").trim().toUpperCase();
  ...
  // 5. Función para obtener URL actual desde RTDB
  const getCurrentUrl = (deviceId: string) => {
    const rtInfo = realtimeStatusMap[deviceId];
    return rtInfo?.url_actual || 'Esperando navegación...';
  };

  // 6. Función para obtener estado online desde RTDB
  const getOnlineStatus = (deviceId: string) => {
    const rtInfo = realtimeStatusMap[deviceId];
  ...
  const getCountByAula = (aula: any) => {
    return tablets.filter(dev => {
      const devAula = String(dev.aulaId || "").trim().toUpperCase();
      const devSecc = String(dev.seccion || "").trim().toUpperCase();
      return devAula === String(aula.aulaId || "").trim().toUpperCase() && 
             devSecc === String(aula.seccion || "").trim().toUpperCase();
    }).length;
  };
    ...
    <div className="min-h-screen bg-[#0a0c10] text-slate-300 font-sans">
      <nav className="fixed left-0 top-0 h-full w-20 bg-[#0f1117] border-r border-slate-800/50 flex flex-col items-center py-8 gap-8 z-50">
        ...
        <div className="flex-1 flex flex-col gap-6 pt-10">
          ...
          <button onClick={() => setActiveSection('teachers')} className={`p-3 rounded-xl transition-all ${activeSection === 'teachers' ? 'text-orange-500 bg-orange-500/10' : 'text-slate-600 hover:text-white'}`}><Users className="w-5 h-5" /></button>
        ...
        <header className="sticky top-0 z-40 bg-[#0a0c10]/80 backdrop-blur-md border-b border-slate-800/50 px-8 py-6 flex justify-between items-center">
          <div>
            ...
            <h1 className="text-2xl font-black italic uppercase text-white tracking-tighter">
              {institutionName} <span className="text-slate-500 font-light ml-2">/ {activeSection === 'teachers' ? 'MONITOR DOCENTE' : activeSection.toUpperCase()}</span>
            </h1>
          </div>
        ...
        <div className="p-8 max-w-[1600px] mx-auto grid grid-cols-12 gap-8">
          ...
          <div className="col-span-12 lg:col-span-8">
            <div className="bg-[#0f1117] p-10 rounded-[3rem] border border-slate-800 min-h-[600px] relative">
              ...
              {/* Sección: DASHBOARD (Aulas y Alumnos) */}
              {activeSection === 'dashboard' && !selectedAula && (
                <>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {aulas.map((aula) => {
                      const count = getCountByAula(aula);
                      return (
                ...
                <div className="animate-in fade-in slide-in-from-right duration-300">
                  ...
                  <div className="space-y-4">
                    {alumnos.length === 0 ? (
                      ...
                      alumnos.map((alumno) => {
                        const onlineStatus = getOnlineStatus(alumno.id);
                        const isOnline = onlineStatus === "online";
                        const currentUrl = getCurrentUrl(alumno.id);
                        ...
                        return (
                          <div key={alumno.id} className="bg-slate-900/40 border border-slate-800 p-6 rounded-[2rem] flex items-center justify-between group hover:border-orange-500/50 transition-all">
                            <div className="flex items-center gap-5">
                              <div className={`w-14 h-14 rounded-2xl flex items-center justify-center ${isOnline ? 'bg-orange-500 shadow-lg shadow-orange-500/20' : 'bg-slate-800'} text-white`}>
                                <Smartphone size={24} />
                              </div>
                              <div>
                                ...
                                <div className="flex flex-col gap-1">
                                  <div className="flex items-center gap-2 text-[9px] font-bold uppercase italic">
                                    {isOnline ? (
                                      <span className="text-green-500 flex items-center gap-1"><Wifi size={10} /> En Línea</span>
                                    ) : (
                                      <span className="text-red-500 flex items-center gap-1"><WifiOff size={10} /> Desconectado</span>
                                    )}
                                    <span className="text-slate-600">| ID: {alumno.id.slice(-8)}</span>
                                  </div>
                                  <div className="text-[8px] text-blue-400 truncate max-w-[300px]">
                                    🌐 {currentUrl}
                                  </div>
                                </div>
                              </div>
