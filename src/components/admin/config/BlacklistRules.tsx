...
import { ShieldAlert, Plus, X, Loader2, Globe } from 'lucide-react';
 ...
 * RUTA FIRESTORE: configuracion_global/blacklist
 */
export function BlacklistRules() {
  ...
  useEffect(() => {
    ...
    const blacklistDocRef = doc(db, 'configuracion_global', 'blacklist');

    const unsubscribe = onSnapshot(blacklistDocRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        setDoc(blacklistDocRef, { dominios: [] }).catch(console.error);
      }
      setLoading(false);
    }, (error) => {
      console.error(error);
  ...
  const addUrl = async (e: React.FormEvent) => {
    ...
    try {
      const blacklistDocRef = doc(db, 'configuracion_global', 'blacklist');
      await updateDoc(blacklistDocRef, {
        dominios: arrayUnion(cleanUrl)
      });
      setNewUrl('');
      toast.success("DOMINIO AÑADIDO A LA LISTA NEGRA (FIRESTORE)");
    } catch (error) {
      console.error(error);
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 bg-[#0f1117] rounded-[2.5rem] border border-slate-800">
        ...
        <span className="text-[10px] font-black uppercase italic text-slate-500">Sincronizando Blacklist...</span>
      </div>
    );
  }
    ...
    <div className="bg-[#0f1117] border border-slate-800 rounded-[3rem] p-10 shadow-2xl animate-in fade-in duration-500">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-black italic uppercase text-white tracking-tighter">
              Navegación <span className="text-red-500 font-light text-xl">Bloqueada</span>
            </h2>
            <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest mt-1">Filtro de Seguridad Global / Blacklist</p>
          </div>
        </div>
      ...
      <form onSubmit={addUrl} className="flex gap-4 mb-10">
        <input
          ...
          placeholder="DOMINIO O PALABRA CLAVE (EJ: TIKTOK.COM)"
        />
        <Button
          type="submit"
          className="bg-red-600 hover:bg-red-500 text-white font-black h-auto px-8 rounded-xl text-[10px] uppercase italic transition-all shadow-lg shadow-red-600/20"
        >
          <Plus className="w-5 h-5 mr-2" /> Bloquear
        </Button>
      </form>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-h-[400px] overflow-y-auto pr-4 custom-scrollbar">
        {sites.length === 0 ? (
          <div className="col-span-2 py-16 text-center border border-dashed border-slate-800 rounded-[2rem]">
            <p className="text-slate-600 font-black uppercase italic text-xs">No hay dominios bloqueados estáticamente</p>
          </div>
        ) : (
          sites.map((site) => (
            <div
              key={site}
              className="flex items-center justify-between bg-slate-900/40 p-5 rounded-2xl border border-red-900/30 group hover:border-red-500/50 transition-all"
            >
      ...
      <div className="mt-8 pt-6 border-t border-slate-800/50">
        <p className="text-[8px] text-slate-600 font-bold uppercase italic text-center leading-relaxed">
          Cualquier cambio en esta lista bloqueará el acceso en tiempo real a todas las unidades <br />
