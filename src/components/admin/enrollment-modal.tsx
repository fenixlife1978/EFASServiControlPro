...
import { db, rtdb } from '@/firebase/config'; // Usamos db directamente
...
export function EnrollmentModal({
  ...
  const onSubmit: SubmitHandler<Inputs> = async (data) => {
    if (!db || !enrollmentId || !pendingEnrollment) {
    ...
    try {
      await confirmEnrollment({
        firestore: db,        // Pasamos db como firestore
        rtdb,                 // Pasamos la instancia de RTDB
        enrollmentId,
