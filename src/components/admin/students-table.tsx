...
export function StudentsTable({ institutionId, classroomId }: StudentsTableProps) {
    ...
    // 2. ESCUCHA DE INFRACCIONES DESDE alertas_seguridad (RTDB)
    useEffect(() => {
        if (students.length === 0) return;

        // Obtener todos los deviceIds de los estudiantes
        const deviceIds = students.map(s => s.deviceId);
        
        // Escuchar todas las alertas y contar por dispositivo
        ...
        const unsubscribe = onValue(alertasRef, (snapshot) => {
            ...
            // Inicializar contadores para todos los dispositivos
            deviceIds.forEach(id => { counts[id] = 0; });
            
            if (data) {
                // Contar alertas por dispositivo
                Object.values(data).forEach((alert: any) => {
                    const deviceId = alert.deviceId;
                    if (deviceId && counts.hasOwnProperty(deviceId)) {
                        counts[deviceId]++;
                    }
                });
            }
