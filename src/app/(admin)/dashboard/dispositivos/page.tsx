'use client';
import React, { useState, useEffect } from 'react';
import { rtdb, db } from '@/firebase/config'; 
import { ref, query, orderByChild, equalTo, onValue, update } from 'firebase/database';
import { collection, onSnapshot } from 'firebase/firestore';
import { useInstitution } from '../institution-context';
import { Radio, Tablet, Globe, Activity, ShieldCheck, ShieldAlert, Wifi, WifiOff } from 'lucide-react';
import { useRouter } from 'next/navigation';

export default function DispositivosPage() {
  const { institutionId } = useInstitution();
  const [dispositivos, setDispositivos] = useState<any[]>([]);
  const [devicesInfo, setDevicesInfo] = useState<Map<string, any>>(new Map());
  const [loading, setLoading] = useState(true);
  const [now, setNow] = useState(Date.now());
  const router = useRouter();

  // Actualizador de tiempo para el cálculo de "Hace X segundos"
  useEffect(() => {
    const timer = setInterval(() => setNow(Date.now()), 10000);
    return () => clearInterval(timer);
  }, []);

  // 1. Obtener información adicional de dispositivos desde Firestore (alumnos, aulas)
  useEffect(() => {
    if (!institutionId) return;
    
    const devicesRef = collection(db, 'dispositivos');
    const unsubscribe = onSnapshot(devicesRef, (snapshot) => {
      const map = new Map();
      snapshot.docs.forEach(doc => {
        const data = doc.data();
        if (data.InstitutoId === institutionId) {
          map.set(doc.id, {
            alumno_asignado: data.alumno_asignado || 'Sin asignar',
            aulaId: data.aulaId || '—',
            seccion: data.seccion || '—',
            rol: data.rol || 'estudiante'
          });
        }
      });
      setDevicesInfo(map);
    });
    
    return () => unsubscribe();
  }, [institutionId]);

  // 2. Sincronización con RTDB (status_dispositivos)
  useEffect(() => {
    if (!institutionId) return;

    // La tablet escribe en 'status_dispositivos'
    const dbRef = ref(rtdb, 'status_dispositivos'); 
    
    // Escuchar todos los dispositivos y filtrar por institución en el frontend
    // porque el filtro por 'institutoId' puede no estar indexado correctamente
    const unsub = onValue(dbRef, (snapshot) => {
      if (snapshot.exists()) {
        const data = snapshot.val();
        const devicesList = Object.entries(data)
          .map(([key, value]: [string, any]) => ({ 
            id: key, 
            ...value,
            // Asegurar campos por defecto
            admin_mode_enable: value.admin_mode_enable || false,
            shield_mode_enable: value.shield_mode_enable || false,
            lastSeen: value.lastSeen || value.lastConnection || value.ultimoAcceso || 0,
            url_actual: value.url_actual || 'Esperando navegación...'
          }))
          .filter(device => {
            // Filtrar por institución usando el campo correcto
            const instId = device.InstitutoId || device.institutoId;
            return instId === institutionId;
          });
        
        setDispositivos(devicesList);
      } else {
        setDispositivos([]);
      }
      setLoading(false);
    });

    return () => unsub();
  }, [institutionId]);

  // 3. Función para alternar modo técnico (admin_mode_enable)
  const toggleTechMode = async (deviceId: string, currentStatus: boolean) => {
    try {
      const newStatus = !currentStatus;
      
      // Actualizar en RTDB (lo escucha MonitorService)
      await update(ref(rtdb, `status_dispositivos/${deviceId}`), {
        admin_mode_enable: newStatus,
        last_command_ts: Date.now()
      });
      
      // También actualizar en dispositivos/ para consistencia
      await update(ref(rtdb, `dispositivos/${deviceId}`), {
        admin_mode_enable: newStatus,
        last_command_ts: Date.now()
      });
      
    } catch (error) {
      console.error("Error al cambiar modo técnico:", error);
    }
  };

  // 4. Función para alternar modo blindado (shield_mode_enable)
  const toggleShieldMode = async (deviceId: string, currentStatus: boolean) => {
    try {
      const newStatus = !currentStatus;
      
      await update(ref(rtdb, `status_dispositivos/${deviceId}`), {
        shield_mode_enable: newStatus,
        last_command_ts: Date.now()
      });
      
      await update(ref(rtdb, `dispositivos/${deviceId}`), {
        shield_mode_enable: newStatus,
        last_command_ts: Date.now()
      });
      
    } catch (error) {
      console.error("Error al cambiar modo blindado:", error);
    }
  };

  // Lógica de Semáforo de Conexión
  const getStatus = (lastPulse: number) => {
    if (!lastPulse || lastPulse === 0) return { label: 'Sin Datos', color: 'bg-slate-300', pulse: false, icon: WifiOff };
    const diff = now - lastPulse;
    
    // 45 segundos para considerar "En Línea"
    if (diff < 45000) return { label: 'En Línea', color: 'bg-green-500', pulse: true, icon: Wifi };
    if (diff < 90000) return { label: 'Inactivo', color: 'bg-orange-400', pulse: false, icon: WifiOff };
    return { label: 'Desconectado', color: 'bg-red-500', pulse: false, icon: WifiOff };
  };

  const getDeviceInfo = (deviceId: string) => {
    return devicesInfo.get(deviceId) || {
      alumno_asignado: 'Sin asignar',
      aulaId: '—',
      seccion: '—',
      rol: 'estudiante'
    };
  };

  return (
    <div className="p-8 bg-slate-50 min-h-screen font-sans">
      {/* Botón Volver */}
      <button 
        onClick={() => router.back()}
        className="mb-8 flex items-center gap-3 text-slate-400 hover:text-orange-500 font-black uppercase text-[10px] tracking-widest transition-all group"
      >
        <span className="bg-white h-10 w-10 flex items-center justify-center rounded-full shadow-sm group-hover:shadow-orange-200 group-hover:scale-110 transition-all text-lg font-bold italic">←</span>
        Volver al Panel
      </button>

      {/* Título */}
      <header className="mb-12">
        <h1 className="text-5xl font-black italic uppercase text-slate-900 tracking-tighter leading-none">
          Mando de <span className="text-orange-500 text-6xl block sm:inline">Unidades</span>
        </h1>
        <p className="text-slate-400 font-bold uppercase text-[10px] tracking-[0.4em] mt-4 italic flex items-center gap-2">
          <Activity className="w-3 h-3 text-orange-500" /> Monitoreo en Tiempo Real (RTDB)
        </p>
      </header>

      {loading ? (
        <div className="font-black italic text-slate-300 uppercase animate-pulse">Detectando Hardware...</div>
      ) : dispositivos.length === 0 ? (
        <div className="text-center py-20">
          <Tablet className="w-16 h-16 text-slate-300 mx-auto mb-4" />
          <p className="text-slate-400 font-black uppercase text-sm italic">No hay dispositivos vinculados a esta sede</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {dispositivos.map((disp) => {
            const status = getStatus(disp.lastSeen);
            const isTechMode = disp.admin_mode_enable || false;
            const isShieldMode = disp.shield_mode_enable || false;
            const deviceInfo = getDeviceInfo(disp.id);
            const StatusIcon = status.icon;
            
            return (
              <div key={disp.id} className="bg-white p-8 rounded-[3rem] border-2 border-slate-100 shadow-sm transition-all hover:border-orange-200 group relative">
                
                {/* Header Tarjeta: Icono y Status */}
                <div className="flex justify-between items-start mb-8">
                  <div className={`w-16 h-16 rounded-[1.5rem] flex items-center justify-center shadow-inner transition-colors ${
                    isShieldMode ? 'bg-red-500 text-white' : 
                    isTechMode ? 'bg-orange-500 text-white' : 
                    'bg-slate-900 text-white'
                  }`}>
                    <Tablet className="w-8 h-8" />
                  </div>
                  
                  <div className="flex flex-col items-end">
                    <div className="flex items-center gap-2 mb-1">
                      {status.pulse && (
                        <span className="relative flex h-2 w-2">
                          <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-75"></span>
                          <span className="relative inline-flex rounded-full h-2 w-2 bg-green-500"></span>
                        </span>
                      )}
                      <StatusIcon className={`w-3 h-3 ${status.color.replace('bg-', 'text-')}`} />
                      <span className={`text-[11px] font-black uppercase italic ${status.color.replace('bg-', 'text-')}`}>
                        {status.label}
                      </span>
                    </div>
                    <span className="text-[9px] font-bold text-slate-300 uppercase tracking-widest leading-none italic">
                      ID: {disp.id ? disp.id.slice(-8) : 'N/A'}
                    </span>
                  </div>
                </div>

                {/* Info Identificación */}
                <div className="mb-6">
                  <h3 className="text-2xl font-black italic uppercase text-slate-800 leading-tight">
                    {deviceInfo.alumno_asignado}
                  </h3>
                  <p className="text-[9px] font-bold text-orange-500/60 uppercase mt-1 italic tracking-widest">
                    {deviceInfo.aulaId} - {deviceInfo.seccion}
                  </p>
                </div>

                {/* Navegación Activa */}
                <div className="bg-slate-50 rounded-3xl p-5 border border-slate-100 mb-8 group-hover:bg-orange-50/30 transition-colors">
                  <div className="flex items-center gap-2 mb-3">
                    <Globe className="w-3 h-3 text-orange-500" />
                    <span className="text-[10px] font-black uppercase text-slate-400 italic">Actividad en Navegador</span>
                  </div>
                  <p className="text-xs font-bold text-slate-500 truncate italic">
                    {disp.url_actual || 'Esperando navegación...'}
                  </p>
                  {disp.lastSeen && (
                    <p className="text-[8px] text-slate-400 mt-2">
                      Último pulso: {new Date(disp.lastSeen).toLocaleTimeString()}
                    </p>
                  )}
                </div>

                {/* CONTROLES: Modo Técnico y Modo Blindado */}
                <div className="space-y-3">
                  {/* Modo Técnico */}
                  <div className={`p-4 rounded-3xl border-2 transition-all flex items-center justify-between ${isTechMode ? 'bg-orange-50 border-orange-200' : 'bg-white border-slate-100'}`}>
                    <div className="flex items-center gap-3">
                      {isTechMode ? <ShieldCheck className="w-5 h-5 text-orange-500" /> : <ShieldAlert className="w-5 h-5 text-slate-300" />}
                      <span className={`text-[10px] font-black uppercase italic ${isTechMode ? 'text-orange-600' : 'text-slate-400'}`}>
                        {isTechMode ? 'MODO TÉCNICO' : 'ANULACIÓN TÉCNICA'}
                      </span>
                    </div>
                    
                    <button
                      onClick={() => toggleTechMode(disp.id, isTechMode)}
                      className={`w-12 h-6 rounded-full p-1 transition-colors relative ${isTechMode ? 'bg-orange-500' : 'bg-slate-200'}`}
                    >
                      <div className={`bg-white w-4 h-4 rounded-full shadow-sm transition-transform duration-300 transform ${isTechMode ? 'translate-x-6' : 'translate-x-0'}`} />
                    </button>
                  </div>

                  {/* Modo Blindado */}
                  <div className={`p-4 rounded-3xl border-2 transition-all flex items-center justify-between ${isShieldMode ? 'bg-red-50 border-red-200' : 'bg-white border-slate-100'}`}>
                    <div className="flex items-center gap-3">
                      <ShieldAlert className={`w-5 h-5 ${isShieldMode ? 'text-red-500' : 'text-slate-300'}`} />
                      <span className={`text-[10px] font-black uppercase italic ${isShieldMode ? 'text-red-600' : 'text-slate-400'}`}>
                        {isShieldMode ? 'BLOQUEADO' : 'MODO BLINDADO'}
                      </span>
                    </div>
                    
                    <button
                      onClick={() => toggleShieldMode(disp.id, isShieldMode)}
                      className={`w-12 h-6 rounded-full p-1 transition-colors relative ${isShieldMode ? 'bg-red-500' : 'bg-slate-200'}`}
                    >
                      <div className={`bg-white w-4 h-4 rounded-full shadow-sm transition-transform duration-300 transform ${isShieldMode ? 'translate-x-6' : 'translate-x-0'}`} />
                    </button>
                  </div>
                </div>

                {/* Indicador adicional si está bloqueado */}
                {isShieldMode && !isTechMode && (
                  <p className="text-[8px] text-red-500 mt-3 text-center font-bold">
                    🔒 Dispositivo bloqueado - No se puede usar
                  </p>
                )}
                {isTechMode && (
                  <p className="text-[8px] text-orange-500 mt-3 text-center font-bold">
                    ⚙️ Modo técnico activado - Restricciones anuladas
                  </p>
                )}
              </div>
            );
          })}
        </div>
      )}
    </div>
  );
}
